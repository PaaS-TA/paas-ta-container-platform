...
2. [컨테이너 플랫폼 포털 접속](#2)  
    ...
    * [2.2. 컨테이너 플랫폼 포털 사용자 계정 로그인](#2-2)  
3. [컨테이너 플랫폼 포털 구성](#3)
    * [3.1. 컨테이너 플랫폼 포털 사용자 권한 유형](#3-1)
    * [3.2. 컨테이너 플랫폼 포털 메뉴 구성](#3-2)
4. [컨테이너 플랫폼 포털 메뉴 설명](#4)
    * [4.1. Global 메뉴](#4-1)
    * [4.1.1. Overview](#4-1-1)
    * [4.1.2. Clusters](#4-1-2)
    * [4.1.3. Cloud Accounts](#4-1-3)
    * [4.1.4. Instance Code Template](#4-1-4)    
    * [4.2. Clusters 메뉴](#4-2)
    * [4.2.1. Overview](#4-2-1)
    * [4.2.2. Nodes](#4-2-2)
    * [4.2.3. Namespaces](#4-2-3)
    * [4.3. Workloads 메뉴](#4-3)
    * [4.3.1. Deployment](#4-3-1)
    * [4.3.2. Pods](#4-3-2)
    * [4.3.3. ReplicaSets](#4-3-3)    
    * [4.4. Services 메뉴](#4-4)
    * [4.4.1. Services](#4-4-1)
    * [4.4.2. Ingresses](#4-4-2)
    * [4.5. Storages 메뉴](#4-5)
    * [4.5.1. Persistent Volumes](#4-5-1)
    * [4.5.2. Persistent Volume Claims](#4-5-2)
    * [4.5.3. Storage Classes](#4-5-3)
    * [4.6. ConfigMaps 메뉴](#4-6)
    * [4.6.1. ConfigMaps](#4-6-1)     
    * [4.7. Managements 메뉴](#4-7)
    * [4.7.1. Users](#4-7-1)
    * [4.7.2. Roles](#4-7-2)
    * [4.7.3. Resource Quotas](#4-7-3) 
    * [4.7.4. Limit Ranges](#4-7-4)
    * [4.8. Info 메뉴](#4-8)
    * [4.8.1. Access](#4-8-1)
    * [4.8.2. Private Repository](#4-8-2)
...
### <div id='2.2'/>2.2. 컨테이너 플랫폼 포털 사용자 계정 로그인
#### 사용자 회원가입    
...
| :--- | :--- |
| Super Admin | 전체 클러스터 관리 권한  |
| Cluster Admin | 할당된 클러스터 관리 권한  |
| User | 할당된 클러스터 내 네임스페이스 관리 권한|
...
<tbody>
  <tr>
    ...
    <td>Overview</td>
    <td>클러스터 정보 대시보드</td>
    <td rowspan="4">Super Admin, Cluster Admin</td>
  </tr>
  <tr>
    <td>Clusters</td>
    <td>클러스터 정보 관리</td>
...
# <div id='4'/> 4. 컨테이너 플랫폼 포털 메뉴 설명
본 장에서는 컨테이너 플랫폼 포털의 메뉴에 대한 설명을 기술하였다.
                   
<br>

## <div id='4-1'/> 4.1. Global 메뉴
...
### <div id='4-1-2'/> 4.1.2. Clusters
#### <div id='4-1-2-1'/> 4.1.2.1. Clusters 목록 조회
- 클러스터 목록을 조회한다.
![IMG_1_2_1]

<br>

#### <div id='4-1-2-2'/> 4.1.2.2. Clusters 상세 조회
- 클러스터 상세 정보를 조회한다.
![IMG_1_2_2]
          
<br>

#### <div id='4-1-2-3'/> 4.1.2.3. Clusters 생성 
...
#### <div id='4-1-2-4'/> 4.1.2.4. Clusters 등록 
- 클러스터를 등록한다.
...
#### <div id='4-1-2-5'/> 4.1.2.5. Clusters 수정 
- 클러스터 정보를 수정한다.
...
### <div id='4-1-3'/> 4.1.3. Cloud Accounts
#### <div id='4-1-3-1'/> 4.1.3.1. Cloud Accounts 목록 조회
- Cloud Accounts 목록을 조회한다.
![IMG_1_3_1]

<br>

#### <div id='4-1-3-2'/> 4.1.3.2. Cloud Accounts 상세 조회
- Cloud Accounts 정보를 상세 조회한다.
![IMG_1_3_2]
          
<br>

#### <div id='4-1-3-3'/> 4.1.3.3. Cloud Accounts 생성 
...
#### <div id='4-1-3-4'/> 4.1.3.4. Cloud Accounts 수정 
...
#### <div id='4-1-3-5'/> 4.1.3.5. Cloud Accounts 삭제 
...
### <div id='4-1-4'/> 4.1.4. Instance Code Template
...
#### <div id='4-1-4-1'/> 4.1.4.1. Instance Code Template 목록 조회
- Instance Code Template 목록을 조회한다.  
![IMG_1_4_1]

<br>

#### <div id='4-1-4-2'/> 4.1.4.2. Instance Code Template 상세 조회
- Instance Code Template 정보를 상세 조회한다.
![IMG_1_4_2]
          
<br>

#### <div id='4-1-4-3'/> 4.1.4.3. Instance Code Template 생성 
- Instance Code Template를 생성한다.                   
![IMG_1_4_3]

<br>
                  
#### <div id='4-1-4-4'/> 4.1.4.4. Instance Code Template 수정 
...
#### <div id='4-1-4-5'/> 4.1.4.5. Instance Code Template 삭제 
...
## <div id='4-2'/> 4.2. Clusters 메뉴
### <div id='4-2-1'/> 4.2.1. Overview
#### <div id='4-2-1-1'/> 4.2.1.1. Overview 정보 조회
- Namespace, Deployment, Pod, User의 개수와 Deployment, Pod, ReplicaSet의 차트를 조회한다.
![IMG_2_1_1]                   
                   
<br>
                   
#### <div id='4-2-1-2'/> 4.2.1.2. Overview 클러스터 변경
...
### <div id='4-2-2'/> 4.2.2. Nodes
#### <div id='4-2-2-1'/> 4.2.2.1. Node 목록 조회
- Clusters 메뉴의 Nodes를 클릭하여 Node 목록 페이지로 이동한다.
![IMG_2_2_1]  
 
<br>
                   
#### <div id='4-2-2-2'/> 4.2.2.2. Node 상세 조회
- Node 목록에서 Node명을 클릭하여 Node 상세 페이지로 이동한다.
![IMG_2_2_2]                    

<br>

### <div id='4-2-3'/> 4.2.3. Namespaces                   
#### <div id='4-2-3-1'/> 4.2.3.1. Namespace 목록 조회
- Clusters 메뉴의 Namespaces 클릭하여 Namespace 목록 페이지로 이동한다.
![IMG_2_3_1]                    

<br>
                   
#### <div id='4-2-3-2'/> 4.2.3.2. Namespace 상세 조회
- Namespace 목록에서 Namespace명을 클릭하여 Namespace 상세 페이지로 이동한다.
![IMG_2_3_2]                    

<br>
                   
#### <div id='4-2-3-3'/> 4.2.3.3. Namespace 생성
...
#### <div id='4-2-3-4'/> 4.2.3.4. Namespace 수정
...
#### <div id='4-2-3-5'/> 4.2.3.5. Namespace 삭제
- Namespace 상세에서 삭제 버튼을 클릭하여 Namespace를 삭제한다.
![IMG_2_3_5]                    

<br>

## <div id='4-3'/> 4.3. Workloads 메뉴
### <div id='4-3-1'/> 4.3.1. Deployments
#### <div id='4-3-1-1'/> 4.3.1.1. Deployment 목록 조회
- Workloads의 Deployments를 클릭하여 Deployment 목록 페이지로 이동한다.
![IMG_3_1_1]                    

<br>

#### <div id='4-3-1-2'/> 4.3.1.2. Deployment 상세 조회
- Deployment 목록에서 Deployment명을 클릭하여 Deployment 상세 페이지로 이동한다.
![IMG_3_1_2]                    

<br>

#### <div id='4-3-1-3'/> 4.3.1.3. Deployment 생성
- Deployment 목록에서 생성 버튼을 클릭할 시 Deployment 생성 팝업창이 뜬다.
![IMG_3_1_3]                    

<br>

#### <div id='4-3-1-4'/> 4.3.1.4. Deployment 수정
- Deployment 상세에서 수정 버튼을 클릭할 시 Deployment 수정 팝업창이 뜬다.
![IMG_3_1_4]                    

<br>

#### <div id='4-3-1-5'/> 4.3.1.5. Deployment 삭제
...
#### <div id='4-4-2-4'/> 4.4.2.4. Ingress 수정
...
#### <div id='4-7-1-2'/> 4.7.1.2. 클러스터 관리자 상세 조회
- 클러스터 관리자 User ID를 클릭하여 클러스터 관리자 상세 조회 페이지로 이동한다.
![IMG_7_1_2]

<br>  
                   
#### <div id='4-7-1-3'/> 4.7.1.3. 일반 사용자 목록 조회
...
#### <div id='4-7-1-4'/> 4.7.1.4. 일반 사용자 상세 조회
- 일반 사용자 User ID를 클릭하여 일반 사용자 상세 조회 페이지로 이동한다.
![IMG_7_1_4]

<br>  

#### <div id='4-7-1-5'/> 4.7.1.5. User 수정
...
### <div id='4-8-2'/> 4.8.2. Private Repository
#### <div id='4-8-2-1'/> 4.8.2.1. Private Repository 정보 조회
